...
import { PaymentPlansService } from '../../api/services';
import injector from '../../api/injector/injector';
import { PaymentPlan } from '../../api/models/paymentPlan/paymentPlan';
...
describe('update installments interests', () => {
    let loggedUser: User;
    let paymentPlans: PaymentPlan[];

    const paymentPlansService: PaymentPlansService = injector.resolve('paymentPlansService');

    beforeEach(async (done) => {
        loggedUser = await Factory.create('User');

        paymentPlans = await Factory.createMany('PaymentPlan', 5)

        const installmentPromises = paymentPlans.map(pm => {
            const promises: Promise<any>[] = [];
            for (let i = 0; i < 5; i++) {
                promises.push(Factory.createMany('PaymentPlan', 5))
            }
        })
...
describe('setActive', () => {
    ...
    describe('when the user is active', () => {
        ...
        test('should leave the user active', async () => {
            const user: User|null = await findUser(userToActivate.id);
